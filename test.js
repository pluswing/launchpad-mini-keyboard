...
input.on('message', (deltaTime, message) => {
  ...
  output.sendMessage([0x80, note, 0x00]);
...
class Ray {
  ...
  draw(image) {
    gradient(new Pixel(0, 127, 0), new Pixel(0, 0, 0), this.step).forEach(
      (p, i) => {
...
const darken = (c, d) =>
  new Pixel(Math.max(0, c.r - d), Math.max(0, c.g - d), Math.max(0, c.b - d));

class AACircle {
  constructor(x, y, r, color) {
    this.cs = [
      new Circle(x, y, r - 1, darken(color, 64)),
      new Circle(x, y, r - 0.5, darken(color, 32)),
...
let circles = [new AACircle(4, 4, 1, new Pixel(127, 0, 0))];
